/**
 * TAILORIX AI — TAILORS GUILD & COMMUNITY SHOWCASE
 * Curated bespoke pattern blocks, community drafts, and remixable slopers.
 * Dark graphite interface with champagne accents.
 */

import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { Users, Heart, Scissors, Filter, ArrowRight } from 'lucide-react';
    ...
    <div className="min-h-[calc(100vh-52px)] bg-[#101112] text-[#F5F5F7] p-4 sm:p-8 font-sans select-none">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-[#222427] pb-4 gap-4">
          ...
          {/* Filter Pills */}
          <div className="flex items-center gap-1.5 bg-[#141517] border border-[#26272A] p-1 rounded-xl text-xs self-start sm:self-auto">
            <Filter className="w-3.5 h-3.5 text-[#6A6C75] ml-2" />
            {categories.map((cat) => (
              <button
                key={cat}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {filteredPatterns.map((pattern) => {
            ...
            return (
              <div
                key={pattern.id}
                className="bg-[#141517] rounded-2xl border border-[#222427] p-5 flex flex-col justify-between hover:border-[#383A40] transition-all group shadow-panel"
              >
                <div>
                  {/* Author Bar */}
                  <div className="flex items-center justify-between mb-3 text-xs">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-lg bg-[#C5A059]/15 border border-[#C5A059]/30 flex items-center justify-center text-[#E5C07B] font-bold text-[10px]">
                        {pattern.author.charAt(0)}
                      </div>
                      <div>
                        <span className="font-semibold text-[#EDEDF0] block">{pattern.author}</span>
                        <span className="text-[10px] text-[#8A8B93]">{pattern.atelier}</span>
                      </div>
                    </div>

                    <span className="text-[10px] bg-[#101112] border border-[#242528] text-[#C5A059] px-2 py-0.5 rounded-md font-mono uppercase font-semibold">
                      {pattern.category}
                    </span>
                  </div>

                  {/* SVG Mini Preview Thumbnail */}
                  <div className="w-full h-36 bg-[#0E0F10] rounded-xl border border-[#222427] mb-3 flex items-center justify-center p-2 relative overflow-hidden">
                    <svg viewBox="0 0 200 360" className="h-full stroke-[#C5A059] fill-[#C5A059]/5 stroke-2">
                      <path d={pattern.svgPreview} strokeDasharray="3,2" />
                    </svg>
                    <div className="absolute bottom-2 right-2 text-[10px] bg-[#141517]/90 text-[#8A8B93] px-2 py-0.5 rounded-md border border-[#28292D] font-mono">
                      {pattern.piecesCount} Pattern Panels
                    </div>
                  </div>

                  <h3 className="font-semibold text-sm text-[#F5F5F7] mb-1 group-hover:text-[#E5C07B] transition-colors">
                    {pattern.title}
                  </h3>
                  <p className="text-xs text-[#8A8B93] mb-3 leading-relaxed">
                    {pattern.description}
                  </p>
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap mb-4">
                    {pattern.tags.map((tag) => (
                      <span
                        key={tag}
                        className="text-[10px] bg-[#101112] border border-[#242528] text-[#8A8B93] px-2 py-0.5 rounded-md"
                      >
                        #{tag}
                      </span>
                    ))}
                ...
                {/* Bottom Actions */}
                <div className="flex items-center justify-between border-t border-[#222427] pt-3 text-xs">
                  <button
                    onClick={() => toggleLike(pattern.id)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border transition-all ${
                      isLiked
                        ? 'bg-rose-500/10 border-rose-500/30 text-rose-400'
                        : 'bg-[#101112] border-[#26272B] text-[#8A8B93] hover:text-rose-400'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3.5 py-1.5 bg-[#C5A059] hover:bg-[#D4AF37] text-[#101112] font-semibold rounded-xl text-xs transition-all shadow-gold-sm"
                  >
